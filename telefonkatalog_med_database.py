...
def finnPerson(typeSok, sokeTekst): 
    #Opretter SQL-spørringen basert på søketypen"
    query = f"SELECT * FROM personer WHERE {typeSok}=?"
    cursor.execute(query, (sokeTekst,))
    
    # Hent alle samsvarende resultater
    resultater = cursor.fetchall()

    # Sjekker om noe resultater ble funnet 
    if not resultater:
        print("Finner ingen personer")  
    else:
        # Gå gjennom resultatene og skriver ut hver persons informasjon
        for fornavn, etternavn, telefonnummer in resultater:
            print(f"{fornavn} {etternavn} har telefonnummer {telefonnummer}")
